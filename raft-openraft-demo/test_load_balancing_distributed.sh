...
LEADER_NODE=$(curl -s http://10.7.57.143:8001/metrics | jq -r '.data.current_leader // empty')
...
multicast_request() {
    ...
    while [ $retry_count -le $max_retries ]; do
        ...
        # Send to all 3 nodes in parallel (updated IPs)
        curl -s --max-time 10 -o "$body_file_1" -w "%{http_code}" \
            -X POST "http://10.7.57.143:8001/image/steg" \
...
# Generate summary
{
    ...
    echo ""
    echo "Architecture:"
    echo "  • Requests multicast to all 3 nodes (10.7.57.143:8001, 10.7.57.109:8002, 10.7.57.155:8003)"
    echo "  • Followers reject direct requests (503)"
    echo "  • Leader forwards to healthy nodes based on throughput-weighted selection"
