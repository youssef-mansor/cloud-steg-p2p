...
curl -s -X POST http://127.0.0.1:8001/cluster/init \
  -H 'Content-Type: application/json' -d '{}' > /dev/null
echo "✅ Initialized"

echo "📥 Adding nodes as learners..."
curl -s -X POST http://127.0.0.1:8001/cluster/add-learner \
...
RESULT=$(curl -s -X POST http://127.0.0.1:8001/cluster/change-membership \
  -H 'Content-Type: application/json' \
  -d '{"members": [1, 2, 3]}')
...
if echo "$RESULT" | grep -q "undergoing"; then
    ...
    curl -s -X POST http://127.0.0.1:8001/cluster/change-membership \
      -H 'Content-Type: application/json' \
...
echo "📊 Cluster Status:"
echo -n "  Node 1: "
curl -s http://127.0.0.1:8001/metrics | jq -r .data.state
echo -n "  Node 2: "
curl -s http://127.0.0.1:8002/metrics | jq -r .data.state
echo -n "  Node 3: "
curl -s http://127.0.0.1:8003/metrics | jq -r .data.state
